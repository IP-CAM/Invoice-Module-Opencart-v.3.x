...
class ControllerSaleReturn extends Controller {
	...
	public function onCreateRefund() {
	    if(!file_exists("InvoiceSDK/RestClient.php"))
        {
            return;
        }
        require_once "InvoiceSDK/RestClient.php";
        require_once "InvoiceSDK/CREATE_REFUND.php";
        require_once "InvoiceSDK/common/ITEM.php";
        require_once "InvoiceSDK/common/REFUND_INFO.php";

        $this->load->model('sale/return');
        $this->load->model('sale/order');

        $key =  $this->config->get('payment_invoice_key');
        $login =  $this->config->get('payment_invoice_login');

        $client = new RestClient($login, $key);
        $return_id = $this->request->get['return_id'];
        $return = $this->model_sale_return->getReturn($return_id);
        $order = $this->model_sale_order->getOrder($return["order_id"]);
        $order_histories = $this->model_sale_order->getOrderHistories($return["order_id"]);
        $tran_id = "";

        foreach ($order_histories as $history) {
            if($history["status"] == "Pending" and $history["comment"] != null) {
                $comment = $history["comment"];
                preg_match('/(href=("|\')(.+)("|\'))/', $comment, $matches);
                if(!isset($matches[3]))
                {
                    continue;
                }
                $tran_id = str_replace("https://pay.invoice.su/P","",$matches[3]);
        ...
        $create_refund->id = $tran_id;
        $refund_info = new REFUND_INFO();
        $refund_info->currency = "RUB";
        $refund_info->amount = $order["total"];
        $this->load->model('localisation/return_reason');

        $reasons = $this->model_localisation_return_reason->getReturnReasons();

        foreach ($reasons as $reason) {
            if ($reason["return_reason_id"] == $return["return_reason_id"]) {
                $refund_info->reason = $reason["name"];
                break;
            }
        }

        $create_refund->refund = $refund_info;
        $refund = $client->CreateRefund($create_refund);
    }
}